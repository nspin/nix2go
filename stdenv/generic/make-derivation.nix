  ...
  mkDerivation =
    ...
    then abort ("mkDerivation was called with unsupported hardening flags: " + lib.generators.toPretty {} {
      ...
      derivationArg =
        ...
        in
        {
          name = import ../../modify-name.nix (
            # A hack to make `nix-env -qa` and `nix search` ignore broken packages.
            # TODO(@oxij): remove this assert when something like NixOS/nix#1771 gets merged into nix.
            assert validity.handled; name + lib.optionalString
              (stdenv.hostPlatform != stdenv.buildPlatform)
              ("-" + stdenv.hostPlatform.config)
          );

          builder = attrs.realBuilder or stdenv.shell;
          args = attrs.args or ["-e" (attrs.builder or <nixpkgs/pkgs/stdenv/generic/default-builder.sh>)];
          inherit stdenv;
      ...
      validity = import <nixpkgs/pkgs/stdenv/generic/check-meta.nix> {
